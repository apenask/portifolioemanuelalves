...
### Quando aparecer `Invalid API key` (como no seu print)

Esse erro indica que a chave enviada **não é válida para esse projeto Supabase**.

Checklist rápido:
1. No Supabase do projeto correto, vá em **Settings → API**.
2. Copie a **Project URL** e a chave **anon public** (não use service_role no frontend).
3. No Netlify, configure exatamente:
   - `VITE_SUPABASE_URL=https://SEU_REF.supabase.co`
   - `VITE_SUPABASE_ANON_KEY=eyJ...`
4. Remova aspas extras e espaços no início/fim.
5. Faça novo deploy (redeploy) e depois `Ctrl+F5`.
